...
import { sucess_Altentication, userAltentication } from "../redux/sliceAltentication";
import { useNavigate } from "react-router-dom";

function login() {


  //aqui estamos pegando os dados do storage do redux para verificar se estamos logados
  const dataAltentication = useSelector(userAltentication);
  //aqui estamos pegando o hook do react router dom para redirecionar para "/costumer"
  ...
  const { mutate: mutateSignin } = useMutation((key: string) => login(key), {
    onSuccess: (data: any) => {
      dispath(
  ...
  async function handleSignin(data: any) {
    ...
    // setUidFromFirebase(uid.uid as string)
    mutateSignin(data.apiKey);
  }
  ...
  return (
    <div
      style={{
        backgroundImage:
          "linear-gradient(to left bottom, #00816a, #46a0a6, #8fbdd1, #d0dbeb, #ffffff)",
      }}
      className="w-[100%] h-[100vh] m-0  box-border flex flex-col bg-[#d9ece9] "
    >
      <div className=" w-full md:text-[100px] text-[50px] text-[#00816A]  flex justify-center items-center p-3">
        Soccer-Mania <SoccerBall size={66} color="#2f2e41" weight="duotone" />
      </div>
      <div className="w-[100%] h-full md:flex-row  flex flex-col md:justify-around md:items-center  m-0  box-border">
        ...
        <div className="w-full min-w-[300px] flex-1 md:max-w-[300px] flex  flex-col bg-[#ffffff] items-center rounded-[17px] p-[20px] md:border-double md:border-4 md:border-[#00816A]">
          <h1 className="text-5xl text-[#787780] mb-5">Bem vindo!</h1>

          <div className=" flex flex-col justify-center items-center border-spacing-2">
            <p className="mb-4  text-[#ABA9BB] text-[13px]">
            ...
            <span
              ...
              className="w-[100%] max-w-[250px]  h-[40px] rounded-[17px] flex justify-around items-center p-1 bg-[#00816A] cursor-pointer text-[#FFFFFF] text-[14px] mb-5 hover:bg-[#0081696e]"
            >
